...
export default function Goals() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
        ...
        {goals.map((g, i) => {
          ...
          return (
            <motion.div key={g.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              onClick={() => setSelected(g)} className="glass-card rounded-xl p-5 hover-lift cursor-pointer">
              ...
              <button onClick={(e) => { e.stopPropagation(); setSelected(g); }} className="w-full mt-4 py-2 rounded-xl text-xs font-medium transition-all" style={{ backgroundColor: g.color + "20", color: g.color }}>
                + Add Contribution
              </button>
            </motion.div>
          );
      ...
      {/* Detail Modal */}
      <Modal open={!!selected} onClose={() => setSelected(null)} title="Goal Details">
        {selected && (
          <div className="space-y-5">
            ...
            <button onClick={() => { setSelected(null); toast.success("Contribution added!"); }} className="w-full py-2.5 rounded-xl gradient-primary text-primary-foreground text-sm font-medium shadow-blue hover:opacity-90 transition-opacity">
              Add to Goal
            </button>
          </div>
        )}
      ...
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Create New Goal">
        <div className="space-y-4">
          {[{ label: "Goal Name", type: "text", placeholder: "e.g. Emergency Fund" },
            { label: "Target Amount (₹)", type: "number", placeholder: "0" },
            { label: "Current Amount (₹)", type: "number", placeholder: "0" },
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={() => { setAddModal(false); toast.success("Goal created!"); }} className="flex-1 py-2.5 rounded-xl gradient-primary text-primary-foreground text-sm font-medium shadow-blue hover:opacity-90 transition-opacity">Create Goal</button>
          </div>
        </div>
      </Modal>
    </div>
