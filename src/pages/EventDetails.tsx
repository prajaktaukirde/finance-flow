  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate("/events")} className="p-2 rounded-xl hover:bg-muted text-muted-foreground transition-colors">
          <ArrowLeft size={18} />
        </button>
        <div className="flex items-center gap-3 flex-1">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{event.category} · {new Date(event.date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Total Amount", value: `$${event.totalAmount.toFixed(2)}`, icon: "💰" },
              { label: "Per Person", value: `$${totalPerPerson.toFixed(2)}`, icon: "👤" },
          ...
          <GlassCard className="overflow-hidden" delay={3}>
            <div className="flex items-center justify-between p-4 border-b border-border">
              ...
              <button onClick={() => setAddExpenseModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg gradient-primary text-primary-foreground text-xs font-medium shadow-blue hover:opacity-90">
                <Plus size={13} /> Add
              </button>
            </div>
            {event.expenses.map((ex, i) => {
              ...
              return (
                ...
                >
                  <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center text-sm flex-shrink-0 font-bold text-primary">
                    {ex.description[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground">{ex.description}</p>
                    <p className="text-xs text-muted-foreground">
                      Paid by {paidByMember?.name} · {new Date(ex.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                    </p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-sm font-bold text-foreground">${ex.amount.toFixed(2)}</p>
                    <p className="text-xs text-muted-foreground">${splitAmt.toFixed(2)}/person</p>
                  </div>
                </motion.div>
              );
            })}
        ...
        <div className="space-y-4">
          ...
          <GlassCard className="overflow-hidden" delay={4}>
            <div className="flex items-center justify-between p-4 border-b border-border">
              ...
              <button onClick={() => setAddMemberModal(true)} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground">
                <Plus size={14} />
              </button>
            </div>
              ...
              return (
                <div key={m.id} className="flex items-center gap-3 p-3 border-b border-border last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">Paid: ${paid.toFixed(2)}</p>
                  </div>
          ...
          <GlassCard className="p-4" delay={5}>
            <h3 className="font-semibold text-foreground mb-3 flex items-center gap-2">
              <DollarSign size={16} className="text-primary" /> Settlements
            </h3>
                ...
                {settlements.map((s, i) => (
                  <div key={i} className="bg-muted rounded-xl p-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-bold text-foreground">${s.amount.toFixed(2)}</span>
                    </div>
                    <button
                      onClick={() => toast.success(`Settlement of $${s.amount.toFixed(2)} marked as paid!`)}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button className="py-2 rounded-xl border-2 border-border text-muted-foreground text-sm font-medium hover:border-primary/50">Custom Split</button>
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={() => setAddExpenseModal(false)} className="flex-1 py-2.5 rounded-xl border border-border text-sm font-medium text-muted-foreground hover:bg-muted">Cancel</button>
            <button onClick={() => { setAddExpenseModal(false); toast.success("Expense added!"); }} className="flex-1 py-2.5 rounded-xl gradient-primary text-primary-foreground text-sm font-medium shadow-blue hover:opacity-90">Add Expense</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={addMemberModal} onClose={() => setAddMemberModal(false)} title="Add Member">
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="email" placeholder="member@email.com" className="w-full px-3 py-2 text-sm bg-muted rounded-lg border border-border focus:outline-none focus:ring-2 focus:ring-primary/20 text-foreground" />
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={() => setAddMemberModal(false)} className="flex-1 py-2.5 rounded-xl border border-border text-sm font-medium text-muted-foreground hover:bg-muted">Cancel</button>
            <button onClick={() => { setAddMemberModal(false); toast.success("Member added!"); }} className="flex-1 py-2.5 rounded-xl gradient-primary text-primary-foreground text-sm font-medium shadow-blue hover:opacity-90">Add Member</button>
          </div>
        </div>
      </Modal>
    </div>
