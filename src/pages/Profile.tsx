  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <PageHeader title="Profile" subtitle="Manage your personal information">
        <button onClick={() => { setEditing(!editing); if (editing) toast.success("Profile saved!"); }}
          className="flex items-center gap-2 px-4 py-2 rounded-xl gradient-primary text-primary-foreground text-sm font-medium shadow-blue hover:opacity-90">
      ...
      <div className="space-y-4">
        <GlassCard className="p-6" delay={0}>
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Member since {new Date(profile.joinDate).toLocaleDateString("en-US", { month: "long", year: "numeric" })}</p>
            </div>
          </div>
        </GlassCard>
        <GlassCard className="p-6" delay={1}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[{ label: "Full Name", value: profile.name, icon: User },
              ...
              { label: "Monthly Income", value: `$${profile.monthlyIncome.toLocaleString()}`, icon: DollarSign },
              { label: "Savings Rate", value: `${profile.savingsRate}%`, icon: Calendar }].map(({ label, value, icon: Icon }) => (
              <div key={label}>
                <label className="text-xs font-medium text-muted-foreground block mb-1.5 flex items-center gap-1.5"><Icon size={12} />{label}</label>
                {editing ? (
        ...
        <div className="grid grid-cols-3 gap-4">
          {[{ label: "Total Saved", value: "$26,350", color: "text-success" },
