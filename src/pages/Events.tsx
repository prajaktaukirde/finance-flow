    ...
    <div className="p-6 max-w-5xl mx-auto">
        ...
        {events.map((event, i) => {
          ...
          return (
            <motion.div key={event.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              onClick={() => navigate(`/events/${event.id}`)} className="glass-card rounded-xl p-5 hover-lift cursor-pointer">
      ...
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Create New Event">
        <div className="space-y-4">
          {[{ label: "Event Name", type: "text", placeholder: "e.g. Weekend Trip to Goa" },
          ...
          <div>
            ...
            <input type="text" placeholder="Rahul, Priya, Arjun..." className="w-full px-3 py-2 text-sm bg-muted rounded-lg border border-border focus:outline-none focus:ring-2 focus:ring-primary/20 text-foreground" />
          </div>
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={() => { setAddModal(false); toast.success("Event created!"); }} className="flex-1 py-2.5 rounded-xl gradient-primary text-primary-foreground text-sm font-medium shadow-blue hover:opacity-90 transition-opacity">Create Event</button>
          </div>
        </div>
      </Modal>
    </div>
