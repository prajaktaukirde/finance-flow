...
export default function Analytics() {
  ...
  const dailyData = useMemo(() => {
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      days.push({ day: dateStr, spending: daySpending });
    }
    return days;
  }, [transactions]);
  ...
  const netWorthData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      months.push({ month: monthName, netWorth: Math.max(0, cumulative) });
    }
    return months;
  }, [transactions]);
  ...
  const avgDailySpend = dailyData.reduce((s, d) => s + d.spending, 0) / 14;
  const topCategory = categorySpending[0] || { name: "N/A", value: 0 };
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard 
          ...
          trend={{ value: 8.9, label: "this month" }} 
          delay={0} 
        />
        <StatCard 
          title="Avg Daily Spend" 
          value={`₹${Math.round(avgDailySpend).toLocaleString("en-IN")}`} 
          subtitle="Last 14 days" 
          icon={<Activity size={20} />} 
          trend={{ value: -4.2, label: "vs prev period" }} 
        ...
        <StatCard 
          ...
          subtitle={`₹${topCategory.value.toLocaleString("en-IN")} total`} 
